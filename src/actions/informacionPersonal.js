...
export const getInformacionPersonal = () => {
    ...
    return async (dispatch) => {
        ...
        const mapInfo = (el) => {
            ...
            const habitantes = body.habitantes.filter(habitante => habitante.uid === el._id)
            const vehiculos = body.vehiculos.filter(vehiculo => vehiculo.uid === el._id)
            const personal = body.personal.filter(personal => personal.uid === el._id)

            return {
                ...el,
                habitantes: habitantes[0].habitante,
                vehiculos: vehiculos[0].vehiculo,
                personal: personal[0].personal
            }
...
export const updateVehiculo = () => {
    ...
    return async (dispatch, getState) => {
        ...
        if (body.ok) {
            ...
            setTimeout(() => {
                window.location.href = `${import.meta.env.VITE_WINDOW_URL}/#/admin/informacion-personal`;
            }, 500);
        } else {
            Swal.fire({
                icon: "error",
